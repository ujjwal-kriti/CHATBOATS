...
export default function Backlogs() {
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            ...
            if (!res.ok) throw new Error('Failed to fetch academic status');
   ...
   // Flexible mapping to handle different database field names
   const backlogDetails = (status?.backlogSubjects || status?.backlogs || []);
   const repeatedSubjects = status?.repeatedSubjects || [];
   ...
   return (
      <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Total Backlogs */}
            <div className={`border rounded-3xl p-6 shadow-sm transition-all ${hasBacklogs
                  ? 'bg-rose-50 dark:bg-rose-900/10 border-rose-200 dark:border-rose-800'
                  : 'bg-white dark:bg-gray-800 border-slate-200 dark:border-gray-700'
               }`}>
               <div className="flex items-center gap-4 mb-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${hasBacklogs
                  ...
                  <div>
                     ...
                     <p className={`text-3xl font-black ${hasBacklogs ? 'text-rose-600 dark:text-rose-500' : 'text-slate-900 dark:text-white'
                        }`}>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                           ...
                           <tbody className="divide-y divide-slate-100 dark:divide-gray-700">
                              {backlogDetails.map((b, idx) => (
                                 <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                    <td className="px-6 py-5 font-bold text-slate-800 dark:text-gray-100">{b.subjectName || b.subject}</td>
                                    <td className="px-6 py-5 text-center text-sm font-semibold text-slate-500">{getSemesterLabel(b.semester)}</td>
                           ...
                           <div key={idx} className="flex gap-6 items-start relative before:absolute before:left-[19px] before:top-10 before:bottom-[-24px] before:w-0.5 before:bg-slate-100 dark:before:bg-gray-700 last:before:hidden">
                              ...
                              <div className="flex-1 bg-slate-50 dark:bg-gray-900/30 border border-slate-100 dark:border-gray-700 rounded-2xl p-5">
                                 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
                                    <h4 className="font-bold text-slate-800 dark:text-gray-100 text-lg">{subject.subjectName}</h4>
                                    <span className="px-3 py-1 bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 rounded-full text-xs font-black uppercase shadow-sm">
                                       Attempt Count: {subject.attemptCount}
                                    </span>
                                 </div>
                                 <div className="flex items-center gap-2 text-sm text-slate-500 font-medium">
                                    <AlertTriangle className="w-4 h-4 text-slate-400" />
                                    <span>Next Exam Scheduled: <strong className="text-slate-700 dark:text-gray-300">{subject.nextExam}</strong></span>
                                 </div>
                              </div>
                           </div>
                        ))}
            ...
            <div className="lg:col-span-1">
               <div className={`rounded-[2.5rem] p-8 md:p-10 text-white relative overflow-hidden shadow-xl sticky top-24 ${hasBacklogs ? 'bg-gradient-to-br from-rose-600 to-red-800' : 'bg-[#0F172A] border border-slate-800'
                  }`}>
                  {hasBacklogs && (
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                     ...
                     <div className={`p-6 rounded-3xl mt-4 ${hasBacklogs ? 'bg-rose-900/40 border border-rose-400/30' : 'bg-slate-800/50 border border-slate-700/50'
                        }`}>
                        {hasBacklogs ? (
